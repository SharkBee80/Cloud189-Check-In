...
class Ecloud:
    ...
    globals()["MODEL"] = "V2546A"  # iQOO 15 Ultra
    ...
    def login_flow(self):
        ...
        with open('./debug/1.html', 'w', encoding='utf-8') as f:
            f.write(r.text)
            f.close()
        m = re.search(r"https?://[^\s'\"]+", r.text)
        if not m:
            raise Exception("登录跳转 URL 获取失败")
        redirect = m.group().replace('index.html', 'login.html')
        r = self.retry_request("GET", session, redirect, headers=base_headers, timeout=10)
        with open('debug/2.html', 'w', encoding='utf-8') as f:
            f.write(r.text)
            f.close()
